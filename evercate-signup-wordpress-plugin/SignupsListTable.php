...
class SignupsListTable extends WP_List_Table {
	...
	public function get_columns() {
		$columns = array(
			'form_name'    => _x( 'Form name', 'Column label', 'evercate-signup-wordpress-plugin' ),
			'date' => _x( 'Date', 'Column label', 'evercate-signup-wordpress-plugin' ),
			'user-existed' => _x( 'User existed', 'Column label', 'evercate-signup-wordpress-plugin' ),
			'payload'   => _x( 'Payload', 'Column label', 'evercate-signup-wordpress-plugin' ),
			'status' => _x( 'Status', 'Column label', 'evercate-signup-wordpress-plugin' ),
			'error' => _x( 'Error', 'Column label', 'evercate-signup-wordpress-plugin' ),
		);
	...
	protected function get_sortable_columns() {
		$sortable_columns = array(
			...
			'user-existed'   => array( 'status', false ),
		);
	...
	protected function column_status( $item ) {
		...
		return $item['status'] == "Success" ? $item['status'] : sprintf( ' <span style="color:red;">%1$s</span>', $item['status']);
	}
	...
	function prepare_items() {
		...
		$signups = $this->repository->getSignupsList();
		...
		{
			$formName = $signup->form_id;
			foreach($forms as $form)
			{
				if($form->Id == $signup->form_id)
				{
					$formName = $form->Name;
					break;
				}
			...
			$tableData[] = array(
				'form_name'       => $formName,
				'date'    => $signup->time,
				'user-existed' => $signup->user_existed ? "Yes" : "No",
				'payload' => $signup->payload,
				'status'   => $signup->status < 400 ? "Success" : "Failure",
	...
	protected function usort_reorder( $a, $b ) {
		...
		$orderby = ! empty( $_REQUEST['orderby'] ) ? wp_unslash( $_REQUEST['orderby'] ) : 'date'; // WPCS: Input var ok.
